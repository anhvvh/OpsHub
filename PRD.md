...
*Version 1.0 — synthesized from [research.md](research.md) and the scoping decisions made in the grilling session of 2026-07-14. Portfolio MVP: one builder, ~2 working days, AI-assisted.*
...
Postgres (hosted: Neon or Vercel Postgres) via Prisma. Amounts stored as integer öre (SEK minor units). Enums as Prisma enums.
...
model Order {
  ...
  channel      Channel                     // SHOPIFY | AMAZON
  status       OrderStatus                 // Shopify-style lifecycle
  totalOre     Int
  createdAt    DateTime     @default(now())
  updatedAt    DateTime     @updatedAt
  items        OrderItem[]
...
model Payment {
  ...
  provider      Provider                    // STRIPE | KLARNA
...
model OrderEvent {
  id          String   @id @default(cuid())
  orderId     String
  type        String                        // ORDER_PLACED | PAYMENT_RECEIVED | STATUS_CHANGED | ...
  description String
  createdAt   DateTime @default(now())
  order       Order    @relation(fields: [orderId], references: [id])
}

enum Channel       { SHOPIFY AMAZON }
enum OrderStatus   { PENDING_PAYMENT PAID FULFILLED DELIVERED REFUNDED CANCELLED }
enum Provider      { STRIPE KLARNA }
...
1. **Low stock:** 2–3 products seeded below their threshold (e.g. bestseller "Lavender Face Serum" at 4 units, threshold 15).
2. **Payment failure rate rising:** a cluster of failed payments in the last 24–48h that pushes the failure count over threshold.
3. **Revenue declining:** last-7-days revenue seeded meaningfully below the prior 7 days (e.g. −20%).
4. **Product selling unusually well:** one product's last-7-day sales ~3× its prior-week rate.
5. **Refund spike:** refunds in the last 7 days concentrated on one product, well above the prior week's rate.

## Implementation Decisions

- **Stack:** Next.js (React + TypeScript) full-stack app; API routes / server actions keep all secrets (Claude API key, database URL) server-side. Deployed to a live Vercel URL. Postgres via Prisma (SQLite rejected because writes must persist on serverless hosting).
- **Data strategy:** fully seeded/mock data; **no live integrations** with Shopify/Stripe/Klarna/Amazon. Architecture keeps data access behind a thin repository/service layer so a real integration could be swapped in later (documented as future work, not built).
- **Tenancy & auth:** single seeded merchant; **no authentication** — the app opens directly on the Dashboard.
- **Order lifecycle:** Shopify-style vocabulary (`PENDING_PAYMENT → PAID → FULFILLED → DELIVERED`, terminal `REFUNDED`/`CANCELLED`) — the convention research.md identified as the cross-industry baseline.
- **Persisted writes (exactly two):** manual order-status update and manual stock-count adjustment. Both write to Postgres; a status update also appends an `OrderEvent`. Everything else is read-only or ephemeral UI state.
- **AI Assistant architecture — rules decide, LLM phrases tersely:** a deterministic rule engine (pure functions over queried data) decides *when* each of the five insight types fires and computes the underlying numbers; a server-side Claude API call turns those numbers into a **short, number-first headline plus one brief action line (≤ ~8 words)** — e.g. headline `"Refunds +40% — 6 of 9 on Lavender Face Serum"`, action `"Check the latest batch."` The insight text must stay scannable, not a paragraph. If the LLM call fails, the UI falls back to template text built from the same numbers — the demo must never break on an API hiccup.
...
- **Visual direction:** warm, editorial minimalism — muted/warm palette, strong typography, generous whitespace. Explicitly not enterprise-cluttered (SAP/Amazon cautionary examples) and not generic SaaS blue-and-white. Currency formatted as SEK (`1 234 kr`).
- **Deliverables:** live Vercel URL + GitHub repo with a README (problem, approach, screenshots, stack), a case-study write-up of the PM/BA/UX/architecture thinking, and a short demo video/GIF.

## Testing Decisions

**Primary testing seam — the insight rule engine.** The five rule functions are pure (dataset in → fired insights out), which makes them the highest-value, cheapest-to-test seam in the app. Tests feed constructed datasets and assert on external behavior only (which insights fire, with what severity and values) — never on internals.

- Each of the five rules gets: a firing case, a non-firing case, and a boundary case (exactly at threshold).
- Rate-of-change rules get a "no prior-period data" case (must not fire or divide by zero).

**Secondary seam — the two write paths**, tested at the API-route/server-action level against a test database: updating an order status persists and appends the correct timeline event; adjusting stock persists and flips the low-stock evaluation in the same request cycle. Invalid transitions/values are rejected.
...
*Prior art: none — greenfield repo. These two seams establish the testing convention for the codebase.*
...
- **Seed data is a first-class product artifact.** The demo's credibility depends on the data telling a coherent story (a real-feeling month for Halsa Skincare with five discoverable problems). Budget real time for it.
- **Publishing note:** this spec would normally be filed to the project issue tracker with a `ready-for-agent` label; this repo has no tracker (or git) yet, so it lives as `PRD.md` in the project root. Initialize git before the build starts.
