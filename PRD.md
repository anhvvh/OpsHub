...
*Version 1.4 — synthesized from [research.md](research.md) and the scoping decisions made in the grilling session of 2026-07-14; revised 2026-07-17 to reconcile the Database Model, Sample Data, and Implementation Decisions with the imported design prototype and the resulting build (schema fields, product catalogue, worked examples); revised again 2026-07-17 after building and testing the AI-insight rule engine and the two write-path server actions test-first (§Testing Decisions now reflects the real suite, not a plan); revised again 2026-07-17 after designing the versioned Supabase migration set (`supabase/migrations/`) with business-rule constraints and an idempotent seed script; revised again 2026-07-17 after repointing all six screens from mock data to live Prisma queries and wiring the two write actions into the real UI — `src/lib/mock.ts` is deleted, `Data strategy` above now describes the real architecture, not a plan. Portfolio MVP: one builder, ~2 working days, AI-assisted.*
...
**Business-rule constraints (beyond the shapes shown above):** every money and quantity column has a non-negative `CHECK` (`priceOre`, `stockQuantity`, `totalOre`, `amountOre`, etc.); `OrderItem.quantity` must be `> 0`; SKU/order-number/payment-number/status/channel/provider/type columns reject blank strings. `promoPriceOre`, when set, must be `<= priceOre` — a promotion is a discount, never a markup (inferred from every worked example in this PRD; not stated in so many words, flagged for confirmation). `OrderItem` additionally has a `UNIQUE ("orderId", "productId")` constraint: a repeat purchase of the same product within one order increases `quantity` on the existing line rather than adding a duplicate row. Deliberately **not** added: `CHECK` constraints enumerating the allowed values of `status`/`channel`/`provider`/`type` — that would silently reintroduce the exact migration-on-every-new-status friction the `String`-not-`enum` decision above exists to avoid.

**Source of truth for the schema:** versioned SQL migrations in `supabase/migrations/` (`0001_init_schema.sql`, `0002_constraints_and_indexes.sql`, `0003_enable_rls.sql`, `0004_lock_down_prisma_migrations.sql`), applied via the Supabase migration tooling — never hand-edited in the Dashboard. `supabase/seed.sql` loads a small, fast, hand-picked demo dataset (12 products, 9 orders mirroring `src/lib/mock.ts`) idempotently: `Product`/`Payment`/`Order` conflict-check on their natural unique key (`sku` / `paymentNumber` / `orderNumber`) with `ON CONFLICT DO NOTHING`; each order's items and timeline events are chained off that order's own `INSERT ... RETURNING id`, so a re-run that finds the order already present inserts zero child rows. Verified by running the seed twice and confirming identical row counts. This is separate from and complementary to `prisma/seed.ts`'s larger randomized dataset (~300-500 orders) — the two are not both needed at once.
...
- **Data strategy:** seeded Postgres (`prisma/seed.ts`, ~540 orders across 30 days), read live on every request — **no live integrations** with Shopify/Stripe/Klarna/Amazon, and no separate backend. All six screens are async Server Components that query Prisma directly and render server-side; the database connection string never reaches the browser, the same guarantee a separate API layer would exist to provide, at zero extra moving parts. The UI was originally built against a typed mock module (`src/lib/mock.ts`) shaped identically to the Prisma models; that module is now deleted — every screen reads real data, and the mock shapes only live on as the type contracts the query layer still satisfies.
- **Read queries are consolidated in `src/lib/queries/`** (`dashboard.ts`, `orders.ts`, `inventory.ts`, `payments.ts`), mirroring the existing `src/lib/actions/` convention for writes — one place per domain, all routed through the single Prisma client in `src/lib/db.ts`. No component ever imports `@prisma/client` directly. Since OpsHub is explicitly single-merchant (no `Shop`/org table, no auth), there is no tenant key to scope queries by yet; `src/lib/db.ts` is where that filter would be added if that ever changes.
- **The dashboard's AI Insights panel calls the real pipeline** (`gatherInsightInputs` → `computeInsights` → `phraseInsights` from `src/lib/insights/`) on every load — the same tested rule engine from §Testing Decisions, not a duplicate. The product-detail page's "AI suggestion" card does the same: it calls `lowStockRule` directly with the page's live (possibly just-edited) stock count, rather than a separate hand-written "reorder ~X units" formula — one tested implementation, two screens.
...
- **Persisted writes (exactly two):** manual order-status update and manual stock-count adjustment, both wired into the real UI (`StatusChips`, `StockAdjuster`) and calling the tested Server Actions directly — not local React state standing in for them. Both write to Postgres; a status update also appends an `OrderEvent`. Everything else is read-only or ephemeral UI state. `adjustStock` takes the new absolute quantity (not a relative delta) and rejects anything that isn't a whole number ≥ 0 — the simpler of the two designs, chosen because OpsHub is single-user with no concurrent-edit risk to guard against. Both actions return a result object (`{ok: true, data}` or `{ok: false, error}`) rather than throwing, and both reject an unknown order/product id without crashing. Wiring the stock stepper to a real round trip surfaced a stale-closure race the mock version never could: two rapid clicks before the first write's response re-rendered `stock` both computed `stock + 1` from the same pre-write value, netting +1 instead of +2. Fixed by disabling the stepper for the duration of the pending write — the same pattern any button that fires a server action while showing its own live result should follow.
